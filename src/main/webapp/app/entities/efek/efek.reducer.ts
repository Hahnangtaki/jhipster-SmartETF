...
import { IEfek, defaultValue } from 'app/shared/model/efek.model';
...
const initialState = {
  loading: false,
  errorMessage: null,
  entities: [] as ReadonlyArray<IEfek>,
  entity: defaultValue,
  updating: false,
  updateSuccess: false
};

export type EfekState = Readonly<typeof initialState>;

// Reducer

export default (state: EfekState = initialState, action): EfekState => {
  switch (action.type) {
...
export const getEntities: ICrudGetAllAction<IEfek> = (page, size, sort) => ({
  type: ACTION_TYPES.FETCH_EFEK_LIST,
  payload: axios.get<IEfek>(`${apiUrl}?cacheBuster=${new Date().getTime()}`)
});

export const getEntity: ICrudGetAction<IEfek> = id => {
  const requestUrl = `${apiUrl}/${id}`;
  return {
    type: ACTION_TYPES.FETCH_EFEK,
    payload: axios.get<IEfek>(requestUrl)
  };
};

export const createEntity: ICrudPutAction<IEfek> = entity => async dispatch => {
...
export const updateEntity: ICrudPutAction<IEfek> = entity => async dispatch => {
  const result = await dispatch({
...
export const deleteEntity: ICrudDeleteAction<IEfek> = id => async dispatch => {
  const requestUrl = `${apiUrl}/${id}`;
  const result = await dispatch({
